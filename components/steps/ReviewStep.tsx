...
function EmailBody({ body }: { body: string }) {
  // Render anchor tags as styled clickable links
  const rendered = body
    .replace(
      /<a\s+href="([^"]*)"([^>]*)>(.*?)<\/a>/gi,
      (_m, href, _attrs, text) => {
            ...
            {generated.map((g, i) => (
              ...
              >
                ...
                <div className="ml-4 mt-0.5 text-[10px]">
                  {g.demoUrl
                    ? <span className="text-grn">● Demo live</span>
                    : <span className="text-red">● No demo</span>
                  }
                </div>
              </div>
            ))}
        ...
        <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {selected.demoUrl ? (
                  <a
                    ...
                    className="text-xs text-grn hover:underline"
                  >
                    Open live demo ↗
                  </a>
                ) : (
          ...
          {!showDemo ? (
                  ...
                  <div key={ei} className="bg-sur border border-bor rounded-xl overflow-hidden">
                    <div className="px-4 py-2.5 bg-sur2 border-b border-bor flex items-center justify-between">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-mono ${
                        over ? 'bg-red/15 text-red' : 'bg-bor text-mut'
                      }`}>
                        {wc}w
                      </span>
                    </div>
            ...
            <div className="bg-sur border border-bor rounded-xl overflow-hidden">
              ...
              {!selected.demoUrl && (
                <div className="px-4 py-4 text-sm text-red bg-red/5 border-b border-bor">
                  Demo deploy failed for this lead. Go back to Generate and regenerate it individually.
                </div>
              )}
              {selected.demoHtml && (
                <iframe
